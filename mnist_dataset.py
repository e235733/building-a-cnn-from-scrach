...
class MnistDataset:
    def __init__(self, n_samples=2000, cache_file='mnist.npz'):
        if os.path.exists(cache_file):
            ...
            X_all = mnist.data.astype(xp.float32) / 255.0
            Y_all = xp.identity(10)[mnist.target.astype(xp.int32)]

            print(f"Saving to {cache_file}...")
            xp.savez_compressed(cache_file, X=X_all, Y=Y_all)
